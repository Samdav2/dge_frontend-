...
export default function VerifiedTab({ onReviewClick }: VerifiedTabProps) {
    ...
    useEffect(() => {
        const fetchKyc = async () => {
            try {
                const res = await fetch("/api/admin/kyc?status=verified");
            ...
            <table className="w-full text-left border-collapse select-none">
                <thead>
                    <tr className="border-b border-slate-50 select-none">
                        ...
                        <th className="py-3 px-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">Verified</th>
                ...
                <tbody className="divide-y divide-slate-50">
                    {items.length > 0 ? items.map((item) => (
                        ...
                        <tr><td colSpan={6} className="py-12 text-center text-slate-400 text-xs italic font-medium">No verified KYC requests found.</td></tr>
                    )}
                </tbody>
            </table>
        </div>
