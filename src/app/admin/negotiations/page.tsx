...
export default function AdminNegotiationsPage() {
    ...
    const handleExportCSV = async () => {
        try {
            const res = await fetch(`/api/admin/negotiations?status=${selectedTab.toLowerCase()}&limit=1000`);
            const data = await res.json();
            if (!data.items || data.items.length === 0) return;

            const headers = ["Service", "Initiator", "Receiver", "Orig. Price", "Proposed Price", "Date", "Status"];
            const rows = data.items.map((it: any) => [
                it.service_name, it.initiator_name, it.receiver_name, it.original_price, it.proposed_price, it.created_at, it.status
            ]);
            ...
            link.setAttribute("download", `negotiations_${selectedTab.toLowerCase()}_export_${new Date().toISOString().split("T")[0]}.csv`);
            link.style.visibility = "hidden";
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        } catch (err) {
            console.error("Failed to export negotiations:", err);
        }
    };
    ...
    useEffect(() => {
        const fetchSummary = async () => {
            try {
                const res = await fetch("/api/admin/negotiations?limit=1");
                const data = await res.json();
                if (data.summary) {
                    setSummary(data.summary);
                }
            } catch (err) {
                console.error("Failed to fetch negotiations summary:", err);
            }
        };
        fetchSummary();
    }, []);
            ...
            <main className="flex-1 flex flex-col h-full overflow-y-auto bg-[#fafafa] select-none">
                ...
                <div className="p-8 space-y-6 flex-1 overflow-y-auto select-none max-w-[1400px] mx-auto w-full animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 select-none">
                        <div className="bg-white p-6 rounded-2xl border border-slate-100 flex flex-col justify-between h-[135px] hover:scale-[1.01] transition-all cursor-pointer shadow-[0_4px_24px_rgba(0,0,0,0.01)]" onClick={() => setSelectedTab("Pending")}>
                            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider leading-tight">Pending Negotiations</span>
                            <div className="flex flex-col leading-none">
                                <span className="text-2xl font-bold tracking-tight text-slate-800">{summary.pending.toLocaleString()}</span>
                                <span className="text-[11px] text-amber-600 font-bold leading-tight mt-2 flex items-center gap-1">Awaiting Review</span>
                            </div>
                        </div>

                        <div className="bg-white p-6 rounded-2xl border border-slate-100 flex flex-col justify-between h-[135px] hover:scale-[1.01] transition-all cursor-pointer shadow-[0_4px_24px_rgba(0,0,0,0.01)]" onClick={() => setSelectedTab("Approved")}>
                            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider leading-tight">Accepted Deals</span>
                            <div className="flex flex-col leading-none">
                                <span className="text-2xl font-bold tracking-tight text-slate-800">{summary.accepted.toLocaleString()}</span>
                                <span className="text-[11px] text-emerald-600 font-bold leading-tight mt-2 flex items-center gap-1">↑ Concluded</span>
                            </div>
                        </div>

                        <div className="bg-white p-6 rounded-2xl border border-slate-100 flex flex-col justify-between h-[135px] hover:scale-[1.01] transition-all cursor-pointer shadow-[0_4px_24px_rgba(0,0,0,0.01)]" onClick={() => setSelectedTab("Rejected")}>
                            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider leading-tight">Rejected Proposals</span>
                            <div className="flex flex-col leading-none">
                                <span className="text-2xl font-bold tracking-tight text-slate-800">{summary.rejected.toLocaleString()}</span>
                                <span className="text-[11px] text-red-600 font-bold leading-tight mt-2 flex items-center gap-1">Failed Matches</span>
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-col select-none leading-tight">
                        <h2 className="text-sm font-bold tracking-tight text-slate-800">Negotiation Logs</h2>
                        <p className="text-xs text-slate-400 font-medium mt-0.5">Global overview of price negotiations on the platform</p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 border-b border-slate-100 select-none w-full overflow-x-auto whitespace-nowrap flex-nowrap pb-0.5">
                        {(["Pending", "Approved", "Rejected"] as const).map((tab) => (
                            ...
                            >
                                {tab === "Approved" ? "Accepted" : tab} ({summary[tab === "Approved" ? "accepted" : tab.toLowerCase() as "pending" | "rejected"]})
                            </button>
                        ))}
                    </div>
