...
export default function ApprovedNegotiationsTab() {
    ...
    const fetchNegotiations = async () => {
        try {
            const res = await fetch("/api/admin/negotiations?status=accepted");
            ...
            <table className="w-full text-left border-collapse select-none">
                ...
                <tbody className="divide-y divide-slate-50">
                    {items.length > 0 ? items.map((item) => (
                        ...
                        <tr><td colSpan={9} className="py-12 text-center text-slate-400 text-xs italic font-medium">No accepted negotiations found.</td></tr>
                    )}
                </tbody>
            </table>
