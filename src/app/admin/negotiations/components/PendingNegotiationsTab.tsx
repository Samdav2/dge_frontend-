...
export default function PendingNegotiationsTab() {
    ...
    const fetchNegotiations = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/admin/negotiations?status=pending");
    ...
    const handleAction = async (id: string, action: "accept" | "reject") => {
        showModal({
            ...
            message: `Are you sure you want to ${action} this negotiation?`,
            onConfirm: async () => {
                hideModal();
                try {
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_4px_24px_rgba(0,0,0,0.01)] flex flex-col justify-between overflow-x-auto select-none relative animate-fade-in mt-4 min-h-[400px]">
            ...
            <table className="w-full text-left border-collapse select-none">
                ...
                <tbody className="divide-y divide-slate-50">
                    {items.length > 0 ? items.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors select-none group">
                        ...
                        <tr><td colSpan={9} className="py-12 text-center text-slate-400 text-xs italic font-medium">No pending negotiations found.</td></tr>
                    )}
                </tbody>
            </table>
            ...
            {activeMenu !== null && (
                ...
                >
                    ...
                    <button 
                        onClick={() => {
                            handleAction(activeMenu, "accept");
                            setActiveMenu(null);
                        }}
                        className="w-full px-3 py-2 text-left text-[10px] font-bold text-slate-600 hover:bg-slate-50 rounded-lg flex items-center gap-2"
                    >
                        <Check size={14} className="text-emerald-500" /> Accept Deal
                    </button>
                    <button 
                        onClick={() => {
                            handleAction(activeMenu, "reject");
                            setActiveMenu(null);
                        }}
                        className="w-full px-3 py-2 text-left text-[10px] font-bold text-slate-600 hover:bg-slate-50 rounded-lg flex items-center gap-2"
                    >
                        <X size={14} className="text-red-500" /> Reject Deal
                    </button>
                    <button 
                        onClick={() => {
